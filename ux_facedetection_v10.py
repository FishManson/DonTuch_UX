...
class Callibration:
	...
	def perform_actions(self):
		if Calib_again == True:
			self.new_callibration()
			time.sleep(0.5)
		if No_Callibration == False:
	...
	def new_callibration(self):
		if Calib_again == True:
			response = messagebox.askyesno(message = "It is needed a new callibration.\nDo you want to continue?", title = "CareInterface")
			if response == 1:
				messagebox.showinfo(message = "New callibration will begin", title = "Careinterface")
			else:
				global No_Callibration
				No_Callibration = True
